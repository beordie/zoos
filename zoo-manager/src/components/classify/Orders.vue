<template>
    <div>
        <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item>目</el-breadcrumb-item>
        </el-breadcrumb>
        ...
        <el-card class="box-card">
            <el-form :model="queryParams" ref="queryParams" :inline="true">
                ...
                <el-form-item prop="subclass">
                    <el-select v-model="queryParams.subclass" filterable remote clearable reserve-keyword placeholder="选择亚纲名">
                        <el-option v-for="(name, index) in subclass" :key="index" :label="name" :value="name"></el-option>
                    </el-select>
                </el-form-item>

                <el-form-item prop="orders">
                    <el-select v-model="queryParams.orders" filterable remote clearable reserve-keyword placeholder="输入目"></el-select>
                </el-form-item>
        ...
        <el-dialog class="classify" title="修改增加" :visible.sync="editVisible" :width="width" :before-close="handleClose">
            <el-card style="text-align: left;">
                ...
                <div class="el-input el-input--medium el-input-group el-input-group--prepend vertical-interval">
                    ...
                    <el-select v-model="editParam.subclass" filterable remote clearable reserve-keyword placeholder="选择亚纲名">
                        <el-option v-for="(name, index) in subclass" :key="index" :label="name" :value="name"></el-option>
                    </el-select>
                </div>
                
                <el-input class="vertical-interval" type="text" placeholder="请输入目名" v-model="editParam.orders" maxlength="10" show-word-limit>
                </el-input>
...
export default {
  ...
  methods: {
    ...
    selectList () {
      this.getTotal()
      this.$http.post(this.$host + 'animal/select/kind/order', this.queryParams
      ).then(res => {
    ...
    getTotal () {
      this.$http.post(this.$host + '/animal/select/count/order', this.queryParams).then(res => {
          if (res.data.code == 200) {
